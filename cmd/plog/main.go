...
func main() {
	...
	if err := run(os.Stdin, os.Stdout, cfg, *module, !*noFold); err != nil {
		fmt.Fprintln(os.Stderr, "plog:", err)
		os.Exit(1)
	}
}

// run drives the pipeline: each line is parsed, enriched, folded, and rendered.
func run(in *os.File, out *os.File, cfg render.PlainConfig, module string, fold bool) error {
