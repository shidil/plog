...
// goGrammar recognizes Go panic/goroutine traces: a "goroutine N [state]:"
// header followed by frame pairs of a column-0 function line and an indented
...
func (goGrammar) detect(s string) bool { return goroutineHeader.MatchString(s) }

// parse extracts the header text and frames from s, or nil if it does not
// contain a recognizable goroutine trace.
func (goGrammar) parse(s, module string) *record.StackTrace {
	loc := goroutineHeader.FindStringIndex(s)
	if loc == nil {
		return nil
	}
	header := strings.TrimRight(s[:loc[0]], "\n")
	lines := strings.Split(s[loc[0]:], "\n")
